...
if [ $# -eq 0 ]
then
	echo "Create GeoViewer Project SilverStripe 3.0 V1.0"
	echo ""
	echo "Use this script to clone a SilverStripe project and switches to branch 3.0."
	echo ""
	echo "usage: create_geoviewer.sh projectname"
	echo ""	
...
echo ""
echo " ($step/$numsteps) Deploy SilverStripe GeoViewer module to $1/geoviewer"
echo ""
step=$(($step+1))
(cd $1; git clone git@github.com:silverstripe-labs/silverstripe-geoviewer.git geoviewer; cd geoviewer; git checkout 3.0)

echo ""
echo " ($step/$numsteps) Deploy SilverStripe Command Pattern module to $1/commandpattern"
echo ""
step=$(($step+1))
(cd $1;git clone git@github.com:silverstripe-labs/silverstripe-commandpattern.git commandpattern)
